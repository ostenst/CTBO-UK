...
def simulate_ctbo(
    ...
    ETS_SCENARIO="Medium",            # [-] str: "High", "Medium", or "Low"
    ...
    diesel_increase_abs = carbon_price * (fuels['diesel']['emission_factor'] / 1000) * 100  # [pence/L]
    petrol_increase_abs = carbon_price * (fuels['petrol']['emission_factor'] / 1000) * 100  # [pence/L]
    gas_increase_abs = carbon_price * (fuels['gas']['emission_factor'] / 1000) * 100        # [pence/therm]
    diesel_increase_pct = (diesel_increase_abs / fuels['diesel']['price']) * 100  # [%]
    petrol_increase_pct = (petrol_increase_abs / fuels['petrol']['price']) * 100  # [%]
    gas_increase_pct = (gas_increase_abs / fuels['gas']['price']) * 100           # [%]
    ...
    results['gas_increase_abs'] = gas_increase_abs        # [pence/therm] time series
    results['diesel_increase_pct'] = diesel_increase_pct  # [%] time series
    results['petrol_increase_pct'] = petrol_increase_pct  # [%] time series
    results['gas_increase_pct'] = gas_increase_pct        # [%] time series
    # Gas price in 2040 and 2050
    idx_2040 = np.where(years == 2040)[0][0]
    results['gas_increase_abs_2040'] = gas_increase_abs[idx_2040]  # [pence/therm]
    results['gas_increase_pct_2040'] = gas_increase_pct[idx_2040]  # [%]
    idx_2050 = np.where(years == 2050)[0][0]
    results['gas_increase_abs_2050'] = gas_increase_abs[idx_2050]  # [pence/therm]
    results['gas_increase_pct_2050'] = gas_increase_pct[idx_2050]  # [%]
    return results
...
if __name__ == "__main__":
    ...
    gas_increase_abs = results['gas_increase_abs']
    diesel_increase_pct = results['diesel_increase_pct']
    petrol_increase_pct = results['petrol_increase_pct']
    gas_increase_pct = results['gas_increase_pct']
