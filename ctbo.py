...
for year in years:
    # Initialize variables that may not be set in all scenarios
    DACCS_capacity = 0
    CSU_cost = 0
    ...
    for idx, plant in macc_foak.iterrows():
        if not plant['invested'] and plant['EUR/tCO2'] < ets_price:
            ...
            print(f"Incentivize plant {plant['site-stack']} in {year} and capture {plant['ktCO2_yr_captured']} ktCO2/yr")
    ...
    print(f"Total emissions in {year}: {total_emissions:.1f} ktCO2/yr (plant: {plant_emissions:.1f}, diffuse: {diffuse_emissions:.1f})")
    ...
    storage_capacity = macc_foak['ktCO2_yr_captured'].where(macc_foak['invested'], 0)
    storage_capacity = storage_capacity.sum()

    if CTBO:
        missing_capacity = ctbo_mandate - storage_capacity
        ...
        while missing_capacity > 0 and i < len(macc_foak):
            ...
            if not plant['invested']:
                ...
                storage_capacity = storage_capacity + plant['ktCO2_yr_captured'] # NOTE: represents point source storage capacity
                missing_capacity = ctbo_mandate - storage_capacity
            i += 1
        ...
        if i > 0:
            ...
            print(f"// Marginal plant in year {year} is {marginal_plant['site-stack']} with a cost of {marginal_plant['EUR/tCO2']} EUR/tCO2")
            CTBO_cost = CSU_cost * storage_capacity # [EUR/tCO2 * ktCO2/yr = kEUR/yr]

        # If still missing capacity, add DACCS backstop technology
        if missing_capacity > 0:
            DACCS_capacity = missing_capacity
            CSU_cost = (450 - ets_price) 
            print(f"--- Add DACCS backstop technology in {year} to capture {DACCS_capacity} ktCO2/yr at a cost of 450 EUR/tCO2")
            CTBO_cost += CSU_cost * DACCS_capacity # [EUR/tCO2 * ktCO2/yr = kEUR/yr]
    ...
    storage_capacity_vec.append(storage_capacity)
...
plt.plot(years, storage_capacity_vec, label='Storage Capacity', linewidth=2)
